...
namespace TCS.ServiceLocator {
    public class ServiceLocator : MonoBehaviour {
        ...
        readonly ServiceManager m_services = new();

        const string K_GLOBAL_SERVICE_LOCATOR_NAME = "ServiceLocator [Global]";
        const string K_SCENE_SERVICE_LOCATOR_NAME = "ServiceLocator [Scene]";

        internal void ConfigureAsGlobal(bool dontDestroyOnLoad) {
            if (global == this) {
                Logger.LogWarning("ServiceLocator.ConfigureAsGlobal: Already configured as global", this);
            }
            else if (global) {
                Logger.LogError("ServiceLocator.ConfigureAsGlobal: Another ServiceLocator is already configured as global", this);
            }
            else {
                global = this;
        ...
        internal void ConfigureForScene() {
            ...
            if (sceneContainers.ContainsKey(scene)) {
                Logger.LogError("ServiceLocator.ConfigureForScene: Another ServiceLocator is already configured for this scene", this);
                return;
            }
        ...
        public static ServiceLocator ForSceneOf(MonoBehaviour mb) {
            ...
            foreach (var go in tmpSceneGameObjects.Where(go => go.GetComponent<ServiceLocatorScene>())) {
        ...
        bool TryGetService<T>(out T service) where T : class => m_services.TryGet(out service);
        bool TryGetService<T>(Type type, out T service) where T : class => m_services.TryGet(out service);
